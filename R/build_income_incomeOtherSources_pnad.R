build_income_incomeOtherSources_pnad <- function(Data){
        ...
        #file_location <-  "C:\\Users\\Rogerio\\Google Drive\\PacotesR\\harmonizePNAD\\inst\\extdata\\crosswalk_pnad_incomeOtherSources.csv"
        crosswalk <- data.table::fread(file_location, colClasses = "numeric", dec =",")
        crosswalk <- crosswalk[year == metadata$year]

        income_vars <- str_split(crosswalk$var_incomeOtherSources,pattern = ";") %>% unlist()
        ...
        Data[, incomeOtherSources := rowSums(sub_data)]
        Data[all_NA_tmp == T, incomeOtherSources := NA]
