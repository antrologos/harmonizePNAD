build_income_rentalIncome_pnad <- function(Data){
        ...
        #file_location <-  "C:\\Users\\Rogerio\\Google Drive\\PacotesR\\harmonizePNAD\\inst\\extdata\\crosswalk_pnad_rentalIncome.csv"
        crosswalk <- data.table::fread(file_location, colClasses = "numeric", dec =",")
        crosswalk <- crosswalk[year == metadata$year]

        income_vars <- str_split(crosswalk$var_rentalIncome,pattern = ";") %>% unlist()
        ...
        Data[, rentalIncome := rowSums(sub_data)]
        Data[all_NA_tmp == T, rentalIncome := NA]
